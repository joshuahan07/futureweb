    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((p) => (
        <button key={p} onClick={() => onChange?.(p)} className={onChange ? 'cursor-pointer' : 'cursor-default'}>
          <Heart className={`w-4 h-4 transition-colors ${p <= count ? 'text-red-400 fill-red-400' : 'text-white/20'}`} />
        </button>
      ))}
    </div>
  );
...
function PriceDisplay({ low, high }: { low?: number | null; high?: number | null }) {
  ...
  return <span className="text-sm text-white/50">{text}</span>;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xl p-4">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-lg font-bold text-white">Add to Wishlist</h3>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/5">✕</button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-sm text-white/50 mb-2 block">Item Name *</label>
            <input type="text" value={item} onChange={(e) => setItem(e.target.value)} required autoFocus
              placeholder="What do you want?"
              className="w-full px-3 py-2.5 rounded-xl glass border border-white/10 text-white placeholder:text-white/30 focus:outline-none focus:border-white/20" />
          </div>
          ...
          {imageUrl ? (
            ...
            <div onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
              ...
              {uploading ? <div className="w-5 h-5 border-2 border-white/40 border-t-transparent rounded-full animate-spin" />
                : <><ImageIcon className="w-5 h-5 text-white/30 mb-1" /><span className="text-xs text-white/30">Drop image or click</span></>}
            </div>
          )}
          ...
          {!imageUrl && <input type="url" placeholder="Or paste image URL..." onChange={(e) => { if (e.target.value.trim()) setImageUrl(e.target.value.trim()); }}
            className="w-full px-3 py-2 rounded-xl glass border border-white/10 text-white placeholder:text-white/30 text-sm focus:outline-none" />}

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-white/50 mb-2 block">Min Price</label>
              <input type="number" value={priceLow} onChange={(e) => setPriceLow(e.target.value)} placeholder="0"
                className="w-full px-3 py-2 rounded-xl glass border border-white/10 text-white focus:outline-none" />
            </div>
            <div>
              <label className="text-sm text-white/50 mb-2 block">Max Price</label>
          ...
          <div>
            <label className="text-sm text-white/50 mb-2 block">Priority</label>
            <div className="flex gap-1">
              {[1, 2, 3, 4, 5].map((p) => (
                <button key={p} type="button" onClick={() => setPriority(p)} className="flex-1 py-2 rounded-lg transition-all">
                  <Heart className={`w-5 h-5 mx-auto ${p <= priority ? 'text-red-400 fill-red-400' : 'text-white/20'}`} />
                </button>
              ))}
            </div>
          </div>
          ...
          <textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Notes (optional)"
            className="w-full px-3 py-2 rounded-xl glass border border-white/10 text-white placeholder:text-white/30 text-sm resize-none focus:outline-none" />

          <button type="submit" disabled={!item.trim()}
            className={`w-full py-3 rounded-xl font-medium transition-all ${
              item.trim() ? 'bg-gradient-to-r from-red-500 to-pink-500 text-white active:scale-95' : 'bg-white/5 text-white/30'
            }`}>
            Add to Wishlist
          </button>
        </form>
...
export default function WantlistPage() {
  ...
  if (!currentUser) return <div className="text-center py-20 text-white/40">Please select a user first.</div>;
  if (!loaded) return <div className="flex items-center justify-center py-20"><div className="w-8 h-8 border-2 border-white/20 border-t-mauve rounded-full animate-spin" /></div>;
        ...
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-full border-2 overflow-hidden flex items-center justify-center"
            ...
            {pfp ? <img src={pfp} alt={ownerName} className="w-full h-full object-cover" />
              : <span className="text-sm font-bold text-white">{ownerName[0]}</span>}
          </div>
          <div>
            <h2 className="text-lg font-semibold text-white">{ownerName}&apos;s Wishlist</h2>
            <p className="text-sm text-white/40">{ownerItems.length} items</p>
          </div>
          {isOwnList && (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <h3 className={`font-medium ${isClaimed ? 'line-through text-white/40' : 'text-white'}`}>{item.item}</h3>
                      ...
                      {item.notes && <p className="text-xs text-white/40 mt-1.5">{item.notes}</p>}
